...
这表明同时过 $A,B,C$ 三点的直线不存在。此时我们希望找出一条最优直线 $y = C+Dx$ ,让这条直线尽可能的拟合 $A,B,C$ 三点. 如下图所示：

![](pic/least-square.png)

从矩阵方程来看$Ax = b$ 无解，设该最优直线为$L$ ，对于该直线，取三个点$p_1,p_2,p_3$ 这三个点都位于直线$L$ 上，并且横坐标分别和点$A,B,C$ 相同，如下图所示。
